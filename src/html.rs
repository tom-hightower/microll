use crate::structs::HTMLToken;
use crate::structs::ParseNode;
use crate::structs::RenderItem;
use crate::structs::RenderState;
use std::collections::HashMap;
use std::str;

fn larse(input_u8: &[u8], begin: usize) -> Result<(Vec<ParseNode>, usize), String> {
    let mut result = Vec::new();
    let mut i: usize = begin;
    'outer: while i < input_u8.len() {
        match input_u8[i] as char {
            '<' => {
                i += 1;
                i = eat_whitespace(input_u8, i, Some(true));
                match input_u8[i] as char {
                    'A'..='Z' | 'a'..='z' | '!' => {
                        let mut tag: Vec<u8> = Vec::<u8>::new();
                        let mut buf_pos: usize = i;
                        let start: usize = i - 1;
                        // Collect tag text and match it to a known tag
                        while !is_whitespace(input_u8[buf_pos])
                            && (input_u8[buf_pos] != b'>')
                            && (input_u8[buf_pos] != b'/')
                        {
                            tag.push(input_u8[buf_pos]);
                            buf_pos += 1;
                        }
                        let html_tag = match_tag(tag.clone());
                        let mut attributes: HashMap<String, String> = HashMap::new();
                        if (input_u8[buf_pos] != b'>')
                            && (html_tag != HTMLToken::Comment)
                            && (html_tag != HTMLToken::DocType)
                        {
                            // Find tag attributes
                            let mut attr_name: String = String::new();
                            let mut attr_val: String = String::new();
                            let mut is_name: bool = true;
                            buf_pos += 1;
                            while (buf_pos < input_u8.len()) && not_close_of_open(input_u8, buf_pos)
                            {
                                match input_u8[buf_pos] as char {
                                    '=' => is_name = !is_name,
                                    ' ' | '\t' | '\r' | '\n' => {
                                        if is_quote(input_u8, buf_pos - 1) {
                                            attributes.insert(attr_name, attr_val);
                                            attr_name = String::new();
                                            attr_val = String::new();
                                            is_name = true;
                                        } else {
                                            if is_name {
                                                if !attr_name.is_empty() {
                                                    if attr_val.len() > 3 {
                                                        attributes.insert(
                                                            attr_name,
                                                            attr_val[1..(attr_val.len() - 1)]
                                                                .to_string(),
                                                        );
                                                    } else {
                                                        attributes.insert(attr_name, attr_val);
                                                    }
                                                    attr_name = String::new();
                                                    attr_val = String::new();
                                                }
                                            } else {
                                                attr_val.push(input_u8[buf_pos] as char);
                                            }
                                        }
                                    }
                                    _ => {
                                        if is_name {
                                            attr_name.push(input_u8[buf_pos] as char);
                                        } else {
                                            attr_val.push(input_u8[buf_pos] as char);
                                        }
                                    }
                                }
                                buf_pos += 1;
                            }
                            if !attr_name.is_empty() {
                                if attr_val.len() > 3 {
                                    attributes.insert(
                                        attr_name,
                                        attr_val[1..(attr_val.len() - 1)].to_string(),
                                    );
                                } else {
                                    attributes.insert(attr_name, attr_val);
                                }
                            }
                        }
                        if buf_pos < input_u8.len() {
                            if input_u8[buf_pos] == b'/' || html_tag == HTMLToken::Void {
                                // self closing tag
                                let node = ParseNode::create(
                                    Vec::new(),
                                    html_tag,
                                    attributes,
                                    Vec::new(),
                                    start,
                                    buf_pos + 1,
                                );
                                result.push(node);
                                if input_u8[buf_pos] == b'/' {
                                    i = buf_pos + 2;
                                } else {
                                    i = buf_pos + 1;
                                }
                            } else if html_tag == HTMLToken::Comment {
                                while !is_closing_comment(input_u8, buf_pos)
                                    && (buf_pos + 3 < input_u8.len())
                                {
                                    buf_pos += 1;
                                }
                                i = buf_pos + 3;
                            } else if html_tag == HTMLToken::DocType {
                                // No closing tag, skip
                                while !(input_u8[buf_pos] == b'>') {
                                    buf_pos += 1
                                }
                                i = buf_pos + 1
                            } else if html_tag == HTMLToken::LineBreak {
                                // No closing tag, cannot have children
                                let node = ParseNode::create(
                                    Vec::new(),
                                    html_tag,
                                    attributes,
                                    Vec::new(),
                                    start,
                                    buf_pos,
                                );
                                result.push(node);
                                i = buf_pos + 1;
                            } else {
                                //close this bad boi and look for text contents/nested tags
                                buf_pos += 1;
                                let mut text = Vec::<u8>::new();
                                let mut children = Vec::<ParseNode>::new();
                                buf_pos = eat_whitespace(input_u8, buf_pos, Some(true));
                                let mut start_text: usize = buf_pos;
                                while (buf_pos < input_u8.len())
                                    && !is_closing_tag(input_u8, buf_pos, &tag)
                                    && !list_item_no_close(input_u8, buf_pos, &tag, &html_tag)
                                {
                                    if input_u8[buf_pos] != b'<'
                                        || html_tag == HTMLToken::Script
                                    {
                                        text.push(input_u8[buf_pos]);
                                    } else {
                                        if !text.is_empty() {
                                            let node = ParseNode::create(
                                                Vec::new(),
                                                HTMLToken::Text,
                                                HashMap::new(),
                                                text,
                                                start_text,
                                                buf_pos - 1,
                                            );
                                            children.push(node);
                                            text = Vec::<u8>::new();
                                        }
                                        match larse(input_u8, buf_pos) {
                                            Ok((new_childs, new_pos)) => {
                                                buf_pos = new_pos - 2;
                                                start_text = buf_pos;
                                                for child in new_childs {
                                                    children.push(child);
                                                }
                                            }
                                            Err(e) => println!("{}", e),
                                        }
                                    }
                                    buf_pos += 1;
                                }
                                if !text.is_empty() {
                                    let node = ParseNode::create(
                                        Vec::new(),
                                        HTMLToken::Text,
                                        HashMap::new(),
                                        text,
                                        start_text,
                                        buf_pos - 1,
                                    );
                                    children.push(node);
                                }
                                if list_item_no_close(input_u8, buf_pos, &tag, &html_tag) {
                                    buf_pos = eat_whitespace(input_u8, buf_pos, Some(true));
                                } else {
                                    while (buf_pos < input_u8.len())
                                        && input_u8[buf_pos] != b'>'
                                    {
                                        buf_pos += 1
                                    }
                                }
                                let node = ParseNode::create(
                                    children,
                                    html_tag,
                                    attributes,
                                    Vec::new(),
                                    start,
                                    buf_pos,
                                );
                                result.push(node);
                                i = buf_pos
                            }
                        } else {
                            // end of file
                            let node = ParseNode::create(
                                Vec::new(),
                                html_tag,
                                HashMap::new(),
                                Vec::new(),
                                start,
                                input_u8.len(),
                            );
                            result.push(node);
                            break 'outer;
                        }
                    }
                    '/' => {
                        //reached the end of nested segment probably
                        break 'outer;
                    }
                    '\n' | ' ' | '\t' | '\r' => {}
                    _ => {
                        // shouldn't be here
                        break 'outer;
                    }
                }
            }
            '\n' | ' ' | '\t' | '\r' => {}
            _ => {
                break 'outer;
            }
        }
        i += 1;
    }
    Ok((result, i))
}

fn match_tag(tag: Vec<u8>) -> HTMLToken {
    let tag_str = str::from_utf8(&tag).unwrap_or_else(|e| {
        panic!("Invalid UTF-8 sequence: {}", e);
    });
    if tag_str.starts_with("!--") {
        return HTMLToken::Comment;
    }
    match tag_str.to_uppercase().as_str() {
        "HTML" => HTMLToken::HtmlStart,
        "HEAD" => HTMLToken::Head,
        "A" => HTMLToken::HyperLink,
        "B" | "STRONG" => HTMLToken::BoldText,
        "BODY" => HTMLToken::Body,
        "BR" | "WBR" | "HR" => HTMLToken::LineBreak,
        "CODE" => HTMLToken::Code,
        "DIV" => HTMLToken::DivSection,
        "H1" | "H2" | "H3" | "H4" | "H5" | "H6" => HTMLToken::Heading,
        "I" | "EM" => HTMLToken::ItalicText,
        "LI" => HTMLToken::ListItem,
        "OL" => HTMLToken::OrderedList,
        "P" => HTMLToken::Paragraph,
        "SCRIPT" => HTMLToken::Script,
        "SPAN" => HTMLToken::Span,
        "TITLE" => HTMLToken::PageTitle,
        "UL" => HTMLToken::UnorderedList,
        "!DOCTYPE" => HTMLToken::DocType,
        "AREA" | "BASE" | "COL" | "COMMAND" | "EMBED" | "IMG" | "INPUT" | "KEYGEN" | "LINK"
        | "META" | "PARAM" | "SOURCE" | "TRACK" | "PATH" => HTMLToken::Void,
        _ => HTMLToken::Unknown,
    }
}

pub fn parse_html(html: &str) -> (Vec<RenderItem>, String) {
    let mut root = ParseNode::new();
    root.tag = HTMLToken::Root;
    root.start_ind = 0;
    root.end_ind = html.len();
    let result;
    match larse(html.as_bytes(), 0) {
        Ok((children, _i)) => {
            result = children;
            for node in result {
                root.children.push(node);
            }
        }
        Err(e) => println!("{}", e),
    }
    let cur_state = RenderState::new();
    build_array(root, Vec::new(), cur_state, String::new())
}

fn build_array(
    node: ParseNode,
    mut ret_vec: Vec<RenderItem>,
    mut cur_state: RenderState,
    title: String,
) -> (Vec<RenderItem>, String) {
    let mut cur_title = title;
    for i in node.children {
        match i.tag {
            HTMLToken::BoldText => {
                cur_state.bold = i.end_ind;
            }
            HTMLToken::HyperLink => {
                cur_state.link = i.end_ind;
                if let Some(link) = i.attributes.get("href") { cur_state.url = link.clone() }
            }
            HTMLToken::LineBreak => {
                let mut break_item = RenderItem::new();
                break_item.line_break = true;
                ret_vec.push(break_item);
            }
            HTMLToken::Code => {
                cur_state.code = i.end_ind;
            }
            HTMLToken::ItalicText => {
                cur_state.italics = i.end_ind;
            }
            HTMLToken::PageTitle => {
                cur_state.title = i.end_ind;
            }
            HTMLToken::Heading => {
                cur_state.heading = i.end_ind;
            }
            HTMLToken::Text
                if !(node.tag == HTMLToken::Unknown || node.tag == HTMLToken::Script)
                    && !is_text_whitespace(&i.text)
                => {
                    let mut item = RenderItem::new();
                    let text = String::from_utf8(i.text.clone());
                    if text.is_ok() {
                        item.text = text.unwrap_or_else(|e| {
                            println!("{}", e);
                            String::new()
                        });
                    }
                    if i.end_ind < cur_state.bold {
                        item.bold = true;
                    }
                    if i.end_ind < cur_state.link {
                        item.link = true;
                        item.url = cur_state.url.clone();
                    }
                    if i.end_ind < cur_state.code {
                        item.code = true;
                    }
                    if i.end_ind < cur_state.italics {
                        item.italics = true;
                    }
                    if i.end_ind < cur_state.title {
                        item.title = true;
                        cur_title = item.text.clone();
                    }
                    if i.end_ind < cur_state.heading {
                        item.heading = true;
                    }
                    ret_vec.push(item);
                }
            _ => {}
        }
        let ret = build_array(i, ret_vec, cur_state.clone(), cur_title.clone());
        ret_vec = ret.0;
        cur_title = ret.1;
    }
    (ret_vec, cur_title)
}

fn eat_whitespace(input_u8: &[u8], mut buf_pos: usize, incl_space: Option<bool>) -> usize {
    match incl_space {
        Some(true) => {
            while (buf_pos < input_u8.len()) && is_whitespace(input_u8[buf_pos]) {
                buf_pos += 1;
            }
        }
        _ => {
            while (buf_pos < input_u8.len()) && is_newline_or_tab(input_u8[buf_pos]) {
                buf_pos += 1;
            }
        }
    }
    buf_pos
}

fn is_text_whitespace(text: &[u8]) -> bool {
    text.len() == 1 && is_whitespace(text[0])
}

fn is_whitespace(c: u8) -> bool {
    matches!(c as char, '\n' | ' ' | '\t' | '\r')
}

fn is_newline_or_tab(c: u8) -> bool {
    matches!(c as char, '\n' | '\r' | '\t')
}

fn is_closing_tag(input_u8: &[u8], buf_pos: usize, tag: &[u8]) -> bool {
    if buf_pos + 2 < input_u8.len() {
        input_u8[buf_pos] == b'<'
            && input_u8[buf_pos + 1] == b'/'
            && input_u8[buf_pos + 2..buf_pos + 2 + tag.len()].eq_ignore_ascii_case(tag)
    } else {
        true
    }
}

fn is_opening_tag(input_u8: &[u8], buf_pos: usize, tag: &[u8]) -> bool {
    if buf_pos + 1 < input_u8.len() {
        input_u8[buf_pos] == b'<'
            && input_u8[buf_pos + 1..buf_pos + 1 + tag.len()].eq_ignore_ascii_case(tag)
    } else {
        true
    }
}

fn is_closing_comment(input_u8: &[u8], buf_pos: usize) -> bool {
    if buf_pos + 2 < input_u8.len() {
        input_u8[buf_pos] == b'-'
            && input_u8[buf_pos + 1] == b'-'
            && input_u8[buf_pos + 2] == b'>'
    } else {
        true
    }
}

fn is_quote(input_u8: &[u8], buf_pos: usize) -> bool {
    input_u8[buf_pos] == b'\"' || input_u8[buf_pos] == b'\''
}

fn not_close_of_open(input_u8: &[u8], buf_pos: usize) -> bool {
    buf_pos < input_u8.len()
        && (input_u8[buf_pos] != b'>'
            && !(input_u8[buf_pos] == b'/' && input_u8[buf_pos + 1] == b'>'))
}

fn list_item_no_close(
    input_u8: &[u8],
    mut buf_pos: usize,
    tag: &[u8],
    html_tag: &HTMLToken,
) -> bool {
    buf_pos = eat_whitespace(input_u8, buf_pos, Some(true));
    *html_tag == HTMLToken::ListItem
        && (is_opening_tag(input_u8, buf_pos, tag)
            || is_closing_tag(input_u8, buf_pos, b"ul")
            || is_closing_tag(input_u8, buf_pos, b"ol"))
}
